...
def calculate_gpu_usage_info(avg_response, sum_response, overall_response):
    ...
    overall_report = f"*LAST 12H EXTERNAL GPU UTILISATION REPORT (IDLE PRIORITY)*\n\n"
    ...
    full_message += f"\nCheck out the full report: https://docs.google.com/spreadsheets/d/1H1JAWXalZv6WUGHCdemAvEpQPEhEIfyfkgGOHtPWEMc/edit#gid=0"
...
def main():
    ...
    data.append(["LAST 12H EXTERNAL GPU UTILISATION REPORT (IDLE PRIORITY)"])
    ...
    spreadsheet = open_and_fill_spreadsheet(data, 'LAST 12H EXTERNAL GPU UTILISATION REPORT (IDLE PRIORITY)')
    # Get the worksheet
