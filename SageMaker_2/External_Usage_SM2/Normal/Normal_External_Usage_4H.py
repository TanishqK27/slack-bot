...
def calculate_gpu_usage_info(avg_response, sum_response, overall_response):
    ...
    overall_report = f"*LAST 4H EXTERNAL GPU UTILISATION REPORT (NORMAL PRIORITY)*\n\n"
    ...
    full_message += f"\nCheck out the full report: https://docs.google.com/spreadsheets/d/1NjEfDmwOXBKwMtvJmhgKrKPZsmgkpqfWoJCVwHdinnU/edit#gid=0"
...
def main():
    ...
    data.append(["LAST 4H EXTERNAL GPU UTILISATION REPORT (NORMAL PRIORITY)"])
    ...
    spreadsheet = open_and_fill_spreadsheet(data, 'LAST 4H EXTERNAL GPU UTILISATION REPORT (NORMAL PRIORITY)')
    # Get the worksheet
