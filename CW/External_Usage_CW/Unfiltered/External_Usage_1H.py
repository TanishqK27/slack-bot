...
def calculate_gpu_usage_info(avg_response, sum_response, overall_response):
    ...
    overall_report = f"*LAST 1H EXTERNAL CW GPU UTILISATION REPORT *\n\n"
    ...
    full_message += f"\nWIP LINK IS WRONG DO NOT CLICK: https://docs.google.com/spreadsheets/d/1d3CcpukRAGbXhJeSi5R7yVrhqvmhqDxSUox6mEY4VdY/edit#gid=0"
...
def main():
    ...
    with ApiClient(configuration) as api_client:
        ...
        sum_response = api_instance.query_metrics(
            ...
            "sum:nvml.power_usage{qos:top,availability-zone:external} by {project}"
        )
    with ApiClient(configuration) as api_client:
        api_instance = MetricsApi(api_client)
        avg_response = api_instance.query_metrics(
            int((datetime.now() + relativedelta(hours=-1)).timestamp()),
            int(datetime.now().timestamp()),
            "avg:nvml.power_usage{qos:top,availability-zone:external} by {project}"
        )
    ...
    with ApiClient(configuration) as api_client:
        ...
        overall_response = api_instance.query_metrics(
            int((datetime.now() + relativedelta(hours=-1)).timestamp()),
            int(datetime.now().timestamp()),
            "abs(avg:nvml.power_usage{qos:top,availability-zone:external})"
        )
    ...
    data.append(["LAST 1H EXTERNAL CW GPU UTILISATION REPORT "])
    ...
    spreadsheet = open_and_fill_spreadsheet(data, 'External Project Usage 1H')
    # Get the worksheet
