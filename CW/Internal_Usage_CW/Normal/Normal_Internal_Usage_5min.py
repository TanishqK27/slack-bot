...
def calculate_gpu_usage_info(avg_response, sum_response, overall_response):
    ...
    overall_report = f"*LAST 5 MIN CW GPU UTILISATION REPORT  (NORMAL PRIORITY)*\n\n"
...
def main():
    ...
    with ApiClient(configuration) as api_client:
        ...
        sum_response = api_instance.query_metrics(
            ...
            "sum:dcgm.power_usage{qos:normal,availability-zone:sagemaker2} by {project}"
        )
    with ApiClient(configuration) as api_client:
        api_instance = MetricsApi(api_client)
    ...
    data.append(["LAST 5 MIN CW GPU UTILISATION REPORT  (NORMAL PRIORITY)"])
