...
def calculate_gpu_usage_info(avg_response, sum_response, overall_response):
    ...
    overall_report = f"*LAST 24H CW GPU UTILISATION REPORT  (NORMAL PRIORITY)*\n\n"
...
def main():
    ...
    with ApiClient(configuration) as api_client:
        ...
        overall_response = api_instance.query_metrics(
            int((datetime.now() + relativedelta(days=-1)).timestamp()),
            int(datetime.now().timestamp()),
            "abs(avg:dcgm.power_usage{ç,availability-zone:cw-prod})"
        )
    ...
    data.append(["LAST 24H CW GPU UTILISATION REPORT  (NORMAL PRIORITY)"])
