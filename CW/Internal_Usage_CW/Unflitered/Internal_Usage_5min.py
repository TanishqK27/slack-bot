...
def calculate_gpu_usage_info(avg_response, sum_response, overall_response):
    ...
    for result in project_info[:10]:
        ...
        message = row_format.format(
            ...
            f"{result['nodes_used']}",
            "N/A"  # No decimal places for hours
        )
        messages.append(message)
    overall_report = f"*LAST 5 MIN CW GPU UTILISATION REPORT *\n\n"
    overall_report += "*Overview:*\n"
    overall_report += "In this report, we present the GPU utilization statistics for the system in the last 5 minutes. " \
    ...
    full_message += f"\nWIP LINK IS WRONG DO NOT CLICK: https://docs.google.com/spreadsheets/d/1B-Od4lKZCBUjxv9Yv8fP2mgC0QuMoSp76x2Upm1zDbg/edit#gid=0"
...
def main():
    ...
    data.append(["LAST 5 MIN CW GPU UTILISATION REPORT "])
